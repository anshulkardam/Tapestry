...
export const ProductFormSchema = z
  .object({
    ...
    sizes: z
      .array(z.enum(sizes))
      .min(1, { message: "Atleast one size is required!" }),
    colors: z
      .array(z.enum(colors))
      .min(1, { message: "Atleast one color is required!" }),
  ...
  .refine(
    (data) => {
      // Ensure every selected color has an image
      return data.colors.every((color) => Boolean(data.images?.[color]));
    },
    {
